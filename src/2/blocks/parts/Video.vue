...
<template>
    <div  class="popup-block__inner">
        ...
        <div class="block-fields">
            <div class="field">
                <div class="name">{{ $t('Video') }}</div>
                <div class="value">
                    <label>
                        <input type="hidden" v-model="block['content']">
                        <input type="file"
                            v-on:change="uploadFile([$event, 'popup_video', card + '/' + card_id + '/' + block_id, {route, card, card_id, block_id, index_group, index_block, block}, 'video'])"    
                        >
                        <div class="view-image">
                            <div class="view-image__body" v-if="block['content']">
                                <b>{{ block['content'] }}</b>
                                <br>
                                <a class="small delete-image" v-on:click.prevent="deleteFile([$event, 'popup_video', card + '/' + card_id + '/' + block_id, {route, card, card_id, block_id, index_group, index_block, block}, false])" href="javascript:void(0);">{{ $t('Delete') }}</a>
                            </div>
...
<script>
    ...
    import templateHtml from './templates/video/Html.vue'
    import templateStyle from './templates/video/Style.vue'
    ...
    export default {
        ...
        methods: {
            ...mapMutations(['uploadFile', 'deleteFile']),

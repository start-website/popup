    ...
    <div class="field">
        ...
        <div class="value">
            <label>
                ...
                <a href="#" v-on:click.prevent="openEdit" v-show="!is_show_edit_groups">
            ...
            <div class="edit-groups" v-if="is_show_edit_groups">
                <div class="edit-groups__content">
                    <div class="edit-groups__delete" v-for="(value, key) in getSettings['routes'][getSettings.selected_route]['popup_card_groups_name']" :key="key">
                        ...
                        <a href="#" v-on:click.prevent="delGroup(key)" :ref="'del-group-' + key">
                            <i class="fa fa-minus-circle"></i><i class="icon-margin">{{ $t('Delete') }}</i>
                        </a>
                        <a href="#" v-on:click.prevent="cancelDelGroup(key)" :ref="'cancel-del-group-' + key" style="display: none;">
                            <i class="fa fa-undo"></i><b><i class="icon-margin">{{ $t('Cancel') }}</i></b>
                        </a>
                    </div>
                    <div class="edit-groups__save" v-for="(group, index) in added_groups" :key="index">
                        <input type="text" v-model="group.name">
                        <a href="#" v-on:click.prevent="saveGroup(index)">
                            <i class="fa fa-book"></i><i class="icon-margin">{{ $t('Save') }}</i>
                        </a>
                    </div>
                    <a href="#" v-on:click.prevent="addGroup">
                        <i class="fa fa-plus-circle"></i><i class="icon-margin">{{ $t('Add') }}</i>
                    </a>
                </div>
